...
const serveurInvestisseur = {
    ...
    addInvestisseur: function (id_discord, id_serveur, client_token) {
    ...
    addServeurInvestisseur: function (id_discord, id_serveur, client_token) {
        if (token !== client_token) {
            return { message: 'Token invalide', status: false };
        }
        ...
        data[id_discord].push(id_serveur);
        fs.writeFileSync(path.resolve(__dirname, '../data/serveurInvestisseur.json'), JSON.stringify(data, null, 4));
        return { message: 'Serveur ajouté à l\'investisseur', status: true };
    },

    // Supprime un serveur d'un investisseur
    deleteServeurInvestisseur: function (id_discord, id_serveur, client_token) {
        if (token !== client_token) {
            return { message: 'Token invalide', status: false };
        }
        ...
        let index = data[id_discord].indexOf(id_serveur);
        if (index > -1) {
